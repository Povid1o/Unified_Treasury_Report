...
class BalanceStructReport(Report):
    ...
    def collect_interactive_args(self) -> Optional[argparse.Namespace]:
        paths = file_discovery.prompt_for_multiple_files(config.BALANCE_STRUCT_SOURCE)
        combine = False
        if len(paths) > 1:
    ...
    def _resolve_inputs(args: argparse.Namespace) -> List[Path]:
        ...
        if args.dates:
            ...
            return [file_discovery.resolve_file_for_date(config.BALANCE_STRUCT_SOURCE, d) for d in dates]
        if args.date:
            return [file_discovery.resolve_file_for_date(config.BALANCE_STRUCT_SOURCE, _parse_date(args.date))]
...
def _combined_output_path() -> Path:
    ...
    return config.BALANCE_STRUCT_OUTPUT_DIR / f"Навигатор_combined_{_date.today().isoformat()}.csv"
