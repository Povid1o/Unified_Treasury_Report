...
def normalize_label(value: Any) -> str:
    """Приводит текст ячейки к виду, пригодному для СРАВНЕНИЯ с эталонной меткой.

    Убирает все пробелы, переводит в нижний регистр, приравнивает "ё" к "е".

    Зачем: в выгрузках одна и та же статья пишется по-разному — "% Активы" и
    "%Активы", с висячим пробелом на конце, с неразрывным пробелом (\\xa0,
    Excel вставляет его сплошь и рядом), с переносом строки внутри ячейки.
    Точное сравнение из-за этого промахивается, и строка молча выпадает из
    отчёта. Сравнение по нормализованному виду от таких различий не зависит.

    Пробелы удаляются целиком, а не схлопываются: так "% Активы" и "%Активы"
    дают одно и то же. Проверено, что на текущих наборах меток всех отчётов
    это не склеивает разные статьи в одну.

    Только для сравнения — в выгрузку всегда идёт эталонное имя из константы
    отчёта, а не то, как статья написана в конкретном файле.
    """
    if value is None:
        return ""
    return _WHITESPACE.sub("", str(value)).casefold().replace("ё", "е")
