...
SETTINGS: List[Setting] = [
    ...
    Setting(
        ...
        help="Колонка «Дюрация» (конечная) выгрузки позиций. Нет (по умолчанию) — "
             "колонка не читается, и «Дюрация-КУАП» в отчёте заполняется только "
             ...
             "идёт в «Дюрация-КУАП», а дюрация по КУАП, если задана, её перебивает.",
    ),
    Setting(
        ...
        help="Через запятую, вида «код портфеля=дюрация в годах», например "
             ...
             "записывается в колонку «Дюрация-КУАП» (duration_target_yrs) "
    ...
    Setting(
        ...
        help="Отчёт старого формата, из которого один раз подтягивается накопленная "
             ...
             "«Динамика TSS» с колонками «Дата» и «Текущий объём». Читается, только "
             "пока в предыдущем выпуске меньше 10 дат истории: дальше история "
             "переносится из выпуска в выпуск, и файл можно удалить. Импорт только "
             "ДОПОЛНЯЕТ: даты, уже накопленные своими запусками, не перезаписываются. "
             "Пусто — не подтягивать.",
    ),
    Setting(
    ...
    Setting(
        ...
        default=r"(?i)состояние\s+лимитов\s+на\s+дату\s*(\d{2}[._-]\d{2}[._-]\d{4})",
        help="Отдельная выгрузка с лимитами по типам портфелей — «Состояние лимитов "
             ...
             "Шаблон нестрогий: регистр не важен, пробелов может быть сколько угодно, "
             "дата — через «_», «.» или «-». Если файла ровно на отчётную дату нет, "
             "берётся ближайший более ранний (с предупреждением в лог).",
    ),
    Setting(
